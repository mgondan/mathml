...
# To do
#
# paste(x, y, z)--> z, y, z (Covered by quote ())
...
The vignette is organized as follows. Section\ 2 presents the technical
background of the package, including the two main classes of Prolog rules for
translating R\ objects to mathematical expressions. Section\ 3 illustrates the
main features of the \pkg{mathml} package, potential issues and workarounds
using examples from the day-to-day perspective of a user. Section\ 4 present a
case study for the use of the package in a scientific report. Section\ 5
concludes with a discussion and ideas for further development.
...
R\ [@murrell2000;@allaire2018]), the \pkg{mathml} package bridges the gap
...
in different formats or ShinyApp webpages: MathML and MathJax. A further, not-yet-mentioned, use of \pkg{mathml} is related to presentations made using
RMarkdown. By adopting \pkg{mathml} researchers or teachers can use the
...
productive use of \pkg{mathml} still comes from more complicated calculations.
...
LaTeX renderers. In its current stage, the LaTeX package \pkg{breqn} [@breqn]
is mostly a proof of concept.

\pkg{mathml} is available for R\ version (…) and later, and can be easily
