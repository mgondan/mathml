...
improvement. However, except for special cases such as regression
models [@equatiomatic] and R's own plotmath annotation, rules for the mapping of
built-in language elements to their mathematical representation are still
lacking. So far, R\ expressions such as `pbinom(k, N, p)` are printed as they
are; pretty mathematical formulae such as \(P_{\mathrm{Bi}}(X \le k; N, p)\)
require explicit LaTeX commands
...
necessary in edge cases. We can include the output in a R\ Markdown document by
specifying `results='asis'` in the R\ code chunk of R\ Markdown, as is done in all
...
R\ to Prolog, but two Prolog functions allow to ring back and evaluate terms
in R. 
...
`math/2` operates like macro that translates one mathematical
...
mathematical formulas in R\ [@murrell2000;@allaire2018]), but most
...
inline formulas, here, the user has to adopt the usual LaTeX notation. The
reason is the missing chunk option results="asis" in inline output from
R\ Markdown, such that the HTML or LaTeX commands are quoted.
...
is mostly a proof of concept. Finally, \pkg{mathml} is one-way road, that is,
translation from LaTeX or HTML back to R is not 
possible [see @latex2r, for an example].
