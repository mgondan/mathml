...
that a special knitr printing function is invoked. We can include the output in
a R\ Markdown document by specifying `results="asis"` in the R\ code chunk of
R\ Markdown, as is done in all examples below.

````{r, results="asis"}
math(term)
````

Similarly, `inline()` produces inline output: `r inline(term)`.
...
more channels. In Schwarz' [-@schwarz1994] model, decision-making builds on a
...
math(call("=", quote(ED_async(tau, c, mu_A, sigma_A, mu_B)), ED_async))
```
...
a proof of concept. Moreover, \pkg{mathml} is one-way road, that is, translation
from LaTeX or HTML back to R is not possible [see @latex2r, for an example].
