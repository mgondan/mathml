...
abstract: >
  ...
  statistical analysis and its verbal or symbolic representation, this new
  feature is a futher step towards reproducible science. User-defined hooks
  allow to extend the package by mapping new variable names and functions to
  new MathML and LaTeX entities. We provide a few working examples that
  illustrate the use of the package in a little scientific report and in an
...
The present R package defines a set of rules for an _automatic_ translation of
...
rules, for example, to represent specific R elements by custom mathematical signs. Moreover, in addition to graphical rendering, the same R expression provides its calculation or evaluation. This feature of the package saves the user's time and allows faster detection of errors.
...
illustrates the main features of the \pkg{mathml} package, potential issues and
workarounds using examples from the day-to-day perspective of the user.
Sections\ 4 and\ 5 present two case studies for the use of the package in a
scientific report and a ShinyApp web page. Section\ 6 concludes with a
...
may be said for the R language. The \pkg{rolog} package bridges this gap by
providing an interface to a SWI-Prolog distribution in R. The communication
between the two systems is mainly in the form of queries from R\ to Prolog, but
two predicates allow Prolog to ring back and evaluate terms in R.

For a first illustration of the package, we consider the binomial probability.
...
readers will remember that the quoted expression is a short form for
...
of the calculation, but an R\ "call" that can eventually be evaluated
[see, e.g., @Wickham2019, for more details on "non-standard evaluation"].
...
The R package \pkg{mathml} can now be used to render the call in
MathML, that is the math dialect for HTML webpages or in MathJax/LaTeX, as shown
below (some of the curly braces are not really needed in this simple example,
but are necessary in edge cases).
...
in the R code chunk, as has been done below. The R function mathout is just a
wrapper that invokes `mathml(...)` for HTML output and `mathjax(...)` for
LaTeX output.
...
At the Prolog end, there is a predicate `math/2` that translates the
call `pbinom(K, N, Pi)` into a "function" `fn/2` with the name `P_Bi`, one
argument `X =< K`, and the two parameters `N` and `Pi`.
...
The low-level predicates `ml/3` and `jax/3` are used to convert mathematical
elements to MathML or LaTeX, respectively.
...
with `N` representing the function's name and `X` its arguments and parameters,
enclosed in parentheses. The flags can be used for context-sensitive
...
web content. The features currently supported are listed below, roughly following
the order proposed by [@murrell2000].

## Basic elements

\pkg{mathml} contains several basic elements used in everyday mathematical expressions, such as superscripts, subscripts, hat, relation and set signs, accents and arrows. Below we provide some examples of their application with commented code. The other basic elements will be displayed in Table \@ref(tab:basic-elements) and Table \@ref(tab:arrows). 
 
Let us start with the function for the linear regression. As the reader may notice, \pkg{mathml} displays Greek letters, accents and superscripts. 


```{r, results='asis'}
term <- quote(hat(y) == hat(b)["0"] + hat(b)["1"]*X["1"])
mathout(term)

# or with greek letters as:
term <- quote(y== beta["0"] + beta["1"]*x["1"] + beta["2"]*x["2"] + ... + beta["i"]*x["i"] + varepsilon["i"])
 
mathout(term)
```

The second example may be particularly of use when we deal with multiple conditions (see \@ref(case-study)). Here, for instance, we present a trivial one with different relation signs. 
...
Note that you may also assign a name to the function. The default name is in fact the greek letter lambda, but with `attributes()$name` the name costumization is performed smoothly.
...
```{r basic-elements, echo=FALSE}
op <- list(subsup=quote(x["a"]^b), lchoose=quote(lchoose(n, k)), sqrt=quote(sqrt(x)), not=quote(!x), different=quote(x != y), mean=quote(mean(xy)), absolute=quote(abs(x)), tilde=quote(X ~ Y), approx=quote(x %~~% y), equiv=quote(x %==% y), congr=quote(x %=~% y), prop=quote(x %prop% y), 'in'=quote(a %in% A), intersect=quote(intersect(A, B)), union=quote(union(A, B)), intersect=quote(is.null(intersect(A, B))))
m <- lapply(op, FUN=mathjax, flags=list(cat=FALSE))

op2 <- list(twosidearrow=quote(x%<->%b), rightarrow=quote(x%->%y), leftarrow=quote(x%<-%y), uparrow=quote(x%up%y), down=quote(x%down%y), iff=quote(x%<=>%y), rArr=quote(x %=>% y), lArr=quote(x %<=% y), dblup=quote(x %dblup% y), dbldown=quote(x %dbldown% y))
m2 <- lapply(op2, FUN=mathjax, flags=list(cat=FALSE))
...
t <- cbind(Operator=op, Output=m, Operator=op2, Output=m2)
knitr::kable(t, caption="Basic elements in mathml", row.names=FALSE, escape=FALSE)
```


## Basic calculations

Basic arithmetic operators and parentheses are translated as they are, as
illustrated below.
...
therefore, \pkg{mathml} defines the extra functions `dot/2`, `nodot/2`,
and `times/2` that produce the respective multiplication signs.
...
Note that for the standard operators with known precedence, \pkg{mathml} is able
to detect if parentheses are needed; for example, a parenthesis is automatically
put around `g + h` in the `nodot`-example and around `n*o` above. However, we
also note an unecessary parenthesis around `l + m` above. This parenthesis
results is a consequence of `quote(a^(b + c))` actually producing a nested
R\ call of the form `'^'(a, (b + c))` instead of `'^'(a, b + c)`:
...
This feature is unfortunate because the extra parenthesis around `b + c` is not
needed and yields to the extra parenthesis. The preferred result is obtained by
using the functional representation `quote('^'(k, l + m))` of the power, or, as
a workaround, curly braces (see last summand above).

## Custom operators

While the parentheses typically follow the precedence in standard infix
operators, undesirable results may be obtained in custom operators.
...
As can be seen, \pkg{mathml} reminds the user that it is not possible to change
the precedence of custom operators in R, and that expressions with such
operators are evaluated strictly from left to right. As in the
previous example, the solution is to either use the functional form of the
operator or a curly brace to enforce the correct operator precedence.
...
A few more custom operators are shown in Table \@ref(tab:custom-operators).

```{r custom-operators, echo=FALSE}
op <- list(cross=quote(x %*% y), dot=quote(x %.% y), kronecker=quote(X %x% Y),
  'integer division'=quote(x %/% y), and=quote(A & B), vel=quote(A | B), xor=quote(xor(A, B)), not=quote(!A))
m <- lapply(op, FUN=mathjax, flags=list(cat=FALSE))
...
knitr::kable(t, caption="Custom operators in mathml", row.names=FALSE, escape=FALSE)
```
...
integration. For mathematical typesetting in the form
of $\int f(x)\, dx$, \pkg{mathml} needs to find out the name of the integration
variable. For that purpose, the underlying Prolog bridge provides a
predicate `r_eval/2` that calls R from Prolog. In the example below, this
predicate is used for a call of `formalArgs(args(sin))`, which returns the names
of the function arguments of `sin`, namely, `x`.
...
by their names, not only by their position in the list of arguments.
Unfortunately, the prolog-powered handler for `integrate/3` is rather rigid; it
accepts only three arguments in that particular order and without names, that
...
order, \pkg{mathml} provides an auxilliary function `canonical(f, drop)` that
reorders the argument list of calls to known functions and, if `drop=TRUE`,
drops the function names.
