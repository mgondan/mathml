...
[see, e.g., @Wickham2019, for more details on "non-standard evaluation"]. This
call can eventually be evaluated with `eval(...)`,
...
R\ function `mathout(...)` is just a wrapper that invokes `mathml(...)` for HTML
output and `mathjax(...)` for LaTeX output.
...
element, namely `fn(..., ...)`. The low-level predicates `ml/3` and `jax/3`
convert mathematical elements to MathML or LaTeX, respectively.
...
\pkg{mathml} contains several basic elements used in everyday mathematical
expressions, such as superscripts, subscripts, hat, relation and set signs,
accents and arrows. Below we provide some examples of their application with
commented code. The other basic elements will be displayed in
Table \@ref(tab:basic-elements) and Table \@ref(tab:arrows). 
