...
```{r arrows, echo=FALSE}
op <- list(twosidearrow=quote(x%<->%b), rightarrow=quote(x%->%y), leftarrow=quote(x%<-%y), uparrow=quote(x%up%y), down=quote(x%down%y), iff=quote(x%<=>%y), rArr=quote(x %=>% y), lArr=quote(x %<=% y), dblup=quote(x %dblup% y), dbldown=quote(x %dbldown% y))
m <- lapply(op, FUN=mathjax, flags=list(cat=FALSE))
...
t <- cbind(Operator=op, Output=m)
knitr::kable(t, caption="Arrows in mathml", row.names=FALSE, escape=FALSE)
```
