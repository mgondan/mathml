...
`mathml` contains several basic elements used in everyday mathematical expressions. 
For example it is possible to print the function for the linear regression as:

```{r, results='asis'}
term <- quote(hat(y) == hat(b)["0"] + hat(b)["1"]*x)
mathout(term)

# or with greek letters as:
term <- quote(y == beta["0"] + beta["1"]*x + ... + varepsilon["i"])
 
mathout(term)
```


_I also wanted to add a condition like 1 < a < 2, but apparently it's not possible_ 
It is also possible to represent a system of equations with different relations. Note that you may also assign a name to the function. The default name is in fact the greek letter lambda, but with `attributes()$name` the name costumization is performed smoothly.
