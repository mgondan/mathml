...
boxes etc. Internally, these decorations are implemented as identitiy functions,
so they can be introduced into R expressions without side-effects. Note that the
font styles only affect the display of identifiers, whereas numbers, character
strings etc. are left untouched.

```{r, results='asis'}
term <- quote(bold(b[x, 5L]) + bold(b[italic(x)]) + italic(ab) + bolditalic(X))
mathout(term)

# hat, overline, box, cancel, prime, phantom, color
term <- quote(1 + -2L + a + abc + "a" + phi + Phi + varphi + hat(b)[i, j]^2L)
mathout(term)
```
