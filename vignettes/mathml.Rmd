...
```{r, results='asis'}
hook(call("ED_sync", expression(c), expression(mu_A), expression(mu_B)), 
     call("E", call(";", quote(D["AB"]), call(",", expression(c), expression(mu_A), expression(mu_B)))))

ED_sync <- function(c, mu_A, mu_B)
  dfrac(c, mu_A + mu_B)

mathout(call("=", quote(E(`;`(D["AB"], `,`(c, mu_A, mu_B)))), ED_sync))
```
...
mathout(call("=", quote(E(`;`(D[tau > 0L], `,`(c, mu_A, sigma_A, mu_B)))),
             ED_async))
```
...
ED <- function(tau, c, mu_A, sigma_A, mu_B, sigma_B)
{
  ...
  if(tau == 0L) return(ED_sync(c, mu_A, mu_B)) ;
  if(tau > 0L) return(ED_async(tau, c, mu_A, sigma_A, mu_B)) ;
  if(tau < 0L) return(ED_async(-tau, c, mu_B, sigma_B, mu_A))
}

mathout(call("=", quote(E(`;`(D[tau], `,`(c, mu_A, sigma_A, mu_B, sigma_B)))),
             ED))
```
...
Then, from Schwarz' [-@schwarz1994] Equations [2] [3] and [10], we calculated the expected first time passage for the respective $\tau$ and the 
parameters $\mu_{\mathrm A} = 0.53, \mu_{\mathrm B} = 1.32, \sigma_{\mathrm A} = 4.43, \sigma_{\mathrm B} = 18.3, c = 100, \mu_{\mathrm M} = 161$
...
For instance, we can see that with $\tau = -167$ the expected decision time is:
...
ED(tau, c, mu_A, sigma_A, mu_B, sigma_B)
```
...
```{r, echo=FALSE}
mu_M = 161
ET(tau, c, mu_A, sigma_A, mu_B, sigma_B, mu_M)
```
...
G <- Vectorize(ET, vectorize.args = 'tau')
