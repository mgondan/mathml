...
expressions (asis, highlight, fix, or ignore), see Table \@ref(tab:mistakes).
...
```{r, results='asis'}
zv <- Vectorize(z, vectorize.args = c('tau', 'm', 's', 'n'))

hook(call("zv", expression(m), expression(s), expression(n), expression(tau), 
        expression(c), expression(mu_A), expression(sigma_A), expression(mu_B), expression(sigma_B), expression(mu_M)), quote(z[tau]))
...
mathout(quote(T^diamond))
mathout(quote(sinpi(alpha)))
```
